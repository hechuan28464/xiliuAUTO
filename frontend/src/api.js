...
export const api = {
  ...
  // === 融合版新增 ===
  // 资产管理
  listAssets: (risk_level, status, limit, offset) =>
    req("GET", `/api/assets${qs({ risk_level, status, limit, offset })}`),
  assetStats: () => req("GET", "/api/assets/stats"),
  // 审计日志
  listAuditLogs: (action, username, limit, offset) =>
    req("GET", `/api/audit${qs({ action, username, limit, offset })}`),
  // 用户管理
  listUsers: () => req("GET", "/api/users"),
  createUser: (data) => req("POST", "/api/users", data),
  updateUserRole: (id, role) => req("PUT", `/api/users/${id}/role`, { role }),
  // 通知配置
  getNotifyConfig: () => req("GET", "/api/notify/config"),
  updateNotifyConfig: (data) => req("PUT", "/api/notify/config", data),
  testNotify: (data) => req("POST", "/api/notify/test", data),
  // HITL 审批
  listPendingApprovals: () => req("GET", "/api/hitl/pending"),
  resolveApproval: (id, data) => req("POST", `/api/hitl/${id}/resolve`, data),
  // 漏洞生命周期
  listLifecycles: (status, limit) =>
    req("GET", `/api/vulns/lifecycle${qs({ status, limit })}`),
  lifecycleStats: () => req("GET", "/api/vulns/lifecycle/stats"),
  updateLifecycle: (findingId, data) => req("PUT", `/api/vulns/lifecycle/${findingId}`, data),
};
