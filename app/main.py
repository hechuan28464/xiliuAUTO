...
async def lifespan(app: FastAPI):
    ...
    # === 融合版增强：确保新模型已注册到 Base.metadata（建表）+ 初始化 RBAC ===
    try:
        import app.auth.rbac  # noqa: F401  → 注册 User/AuditLog 到 Base.metadata
        import app.db.models  # noqa: F401  → 确保 Asset/VulnLifecycle 已注册
        # 再次 create_all 补建新表（init_db 时 rbac 模型可能未导入）
        from app.db.session import engine
        from app.db.models import Base
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)
        from app.auth.rbac import create_default_roles
...
app.include_router(vuln_lifecycle.router)
