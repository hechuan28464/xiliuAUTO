...
from app.tools.mcp import mcp_registry, execute_mcp_tool, format_tool_result, is_high_risk_tool
...
class Worker:
    def __init__(
        ...
        # MCP 工具 schema（融合版：从 YAML 加载的外部安全工具）
        self._mcp_schemas: list[dict] = []
        self._mcp_tool_names: set[str] = set()
        self._init_mcp_tools()
        # 知识库检索器（融合版：RAG 向量检索）
        self._knowledge_retriever = get_retriever()

    def _init_mcp_tools(self) -> None:
        """加载 MCP 外部工具 schema（融合版增强）。"""
        try:
            if not mcp_registry.is_loaded():
                mcp_registry.load_tools()
            self._mcp_schemas = mcp_registry.get_schemas()
            self._mcp_tool_names = set(mcp_registry.list_tool_names())
            if self._mcp_schemas:
                self._emit("mcp_tools_loaded", count=len(self._mcp_schemas),
                           tools=list(self._mcp_tool_names)[:20])
        except Exception as e:
            logger = __import__("logging").getLogger("autohunter.worker")
            logger.warning("MCP 工具加载失败（降级为内置工具）: %s", e)
            self._mcp_schemas = []
    ...
    def _knowledge_block(self) -> str:
        """融合版增强：从 RAG 知识库检索相关经验注入 Worker 上下文。"""
        try:
            if not self._knowledge_retriever.is_available():
                return ""
            host = (self.target_meta or {}).get("host") or ""
            vuln_types = []
            # 从 target_meta 或默认列表获取关注类型
            vt_str = (self.target_meta or {}).get("vuln_types") or ""
            if vt_str and isinstance(vt_str, list):
                vuln_types = vt_str[:3]
            ctx = self._knowledge_retriever.format_worker_context(
    ...
    def run(self) -> WorkerResult:
        ...
        while rounds < max_rounds:
            ...
            try:
                ...
                # 融合版增强：注入 MCP 外部工具 schema
                if self._mcp_schemas:
                    tools += self._mcp_schemas
    ...
    def _dispatch(self, name: str, args: dict, rnd: int) -> dict:
        ...
        if name in self._mcp_tool_names:
            ...
            if result.get("returncode") == 0 and result.get("stdout"):
                try:
                    import hashlib
                    fid = hashlib.md5(f"{name}{rnd}{self.target}".encode()).hexdigest()
                    self._knowledge_retriever.index_bypass(
                        bypass_id=fid,
                        waf_type=name,
            ...
            return {"ok": result["returncode"] == 0, "output": format_tool_result(result),
                    "returncode": result["returncode"], "tool": name}
