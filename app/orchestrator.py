...
class TaskRunner:
            ...
            def _do():
                ...
                try:
                    ...
                    if finding_payload:
                        ...
                        # === 融合版增强：出洞通知 + 知识库索引 ===
                        try:
                            from app.notify import notify_finding
                            notify_finding(finding_payload)
                        except Exception:
                            pass
                        try:
                            ...
                            import hashlib
                            fid = hashlib.md5(
                                f"{finding_payload.get('vuln_type','')}{target_id}{finding_payload.get('title','')}".encode()
                            ).hexdigest()
                            get_retriever().index_finding(
                                finding_id=fid,
                                vuln_type=finding_payload.get("vuln_type", ""),
                                title=finding_payload.get("title", ""),
                                target_url=finding_payload.get("target_url", ""),
                                poc=finding_payload.get("poc", ""),
                                severity=finding_payload.get("severity_claimed", ""),
                            )
                        except Exception:
                            pass
                    if kind == "auth_status":
    ...
    async def _run_killsweep_inner(self, task_id: str, finding_id: str) -> None:
        ...
        async with SessionLocal() as session:
            ...
            # === 融合版增强：通杀确认通知 ===
            try:
                from app.notify import notify_killsweep
                notify_killsweep(res)
            except Exception:
                pass
