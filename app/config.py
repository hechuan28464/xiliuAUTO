...
class MCPConfig(BaseModel):
    """MCP 外部工具配置。"""
    tool_dir: str = os.environ.get("MCP_TOOL_DIR", str(Path(__file__).resolve().parent.parent / "tools" / "mcp"))
    tool_timeout: int = int(os.environ.get("MCP_TOOL_TIMEOUT", "120"))
    max_concurrent: int = int(os.environ.get("MCP_MAX_CONCURRENT", "2"))


class KnowledgeConfig(BaseModel):
    """RAG 知识库配置。"""
    db_path: str = os.environ.get("KNOWLEDGE_DB_PATH", str(Path(__file__).resolve().parent.parent / "data" / "chroma"))
    max_results: int = int(os.environ.get("KNOWLEDGE_MAX_RESULTS", "5"))
    ...
    embedding_timeout: int = int(os.environ.get("EMBEDDING_TIMEOUT", "30"))
