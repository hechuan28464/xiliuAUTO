...
class Asset(Base):
    ...
    __table_args__ = (
        Index("ux_assets_host_task", "host", "task_id", unique=True),
    )

    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=_uuid)
    task_id: Mapped[str] = mapped_column(ForeignKey("tasks.id"), index=True, nullable=True)
    ...
    title: Mapped[str] = mapped_column(String(500), default="")
    tech_stack: Mapped[list] = mapped_column(JSON, default=list)  # ["Nginx", "PHP", "MySQL"]
    org: Mapped[str] = mapped_column(String(300), default="")
    is_edu: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    # unknown / low / medium / high / critical
    risk_level: Mapped[str] = mapped_column(String(20), default="unknown", index=True)
    # discovered / scanning / done / archived
    status: Mapped[str] = mapped_column(String(20), default="discovered", index=True)
    linked_vulns: Mapped[int] = mapped_column(Integer, default=0)  # 关联漏洞数
...
class VulnLifecycle(Base):
    ...
    """
    __tablename__ = "vuln_lifecycles"

    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=_uuid)
    finding_id: Mapped[str] = mapped_column(ForeignKey("findings.id"), index=True, unique=True)
    # submitted / reviewed / accepted / reported / fixed / closed / rejected
    lifecycle_status: Mapped[str] = mapped_column(String(20), default="submitted", index=True)
    # 报告外链（SRC 提交后的 URL）
    report_url: Mapped[str] = mapped_column(String(500), default="")
    # 修复验证记录
    fix_verified: Mapped[bool] = mapped_column(Boolean, default=False)
    fix_verified_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    # 时间线：[{status, timestamp, actor, note}]
